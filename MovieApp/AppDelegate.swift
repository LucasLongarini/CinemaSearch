...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        var numberFetched = 0
        //semaphore for numberFetched to avoid race conditions
        let semaphore = DispatchSemaphore(value: 1)
        //group for main thread
        ...
        movieHelper.downloadMovies(type: .popular, page: 1) { (movies) in
            ...
            semaphore.wait()
                numberFetched += 1
                if(numberFetched == 3){group.leave()}
            semaphore.signal()
        }
        ...
        group.wait()
        //send data to first view controller
        if let firstController = ((self.window?.rootViewController as? UITabBarController)?.viewControllers?[0] as? UINavigationController)?.viewControllers[0] as? DiscoverViewController{
            firstController.allMovieData.newMovies = newMovies
            firstController.allMovieData.popularMovies = popularMovies
